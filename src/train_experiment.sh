...
construct_command() {
  ...
  local output_log_file="${nov30_exp_dirpath}/${log_file}"

  # Construct the command
  cmd="CUDA_VISIBLE_DEVICES=0 TF_FORCE_GPU_ALLOW_GROWTH=true python train.py \
    ...
    --output_results_dirpath=\"${nov30_exp_dirpath}\" \
    --n_epochs=${n_epochs} \
    --lr=${lr} \
...
while IFS= read -r command; do
  ...
  if [ $? -ne 0 ]; then
    echo "Error: Command failed - ${command}" >> "${nov30_exp_dirpath}/error_log.txt"
